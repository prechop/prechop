...
| Delivery confirmed | Warm, clear        | "In transit\! Amaka is bringing your order to you. She'll call when she's close."   |
...
| :----------------- | :------------------------------------------------------------------------------- | :-------------------------- | :------------------------------------------------------------------------------ |
| Runtime            | Node.js 20 LTS \+ TypeScript 5 (strict mode)                                     | _(unchanged)_               | Strong typing, async I/O                                                        |
...
| Cache & Sessions   | Redis (ioredis)                                                                  | _(unchanged, minus BullMQ)_ | OTP storage, refresh tokens, rate-limit counters, slot holds, cron locks        |
...
| Payments           | Paystack                                                                         | _(unchanged)_               | Subaccounts for direct vendor settlement                                        |
| SMS                | **Sendchamp**                                                                    | Termii                      | Nigerian SMS gateway — OTP delivery                                             |
| Email              | Resend                                                                           | _(unchanged)_               | Transactional email — receipts, registration confirmation                       |
| Input Validation   | Zod                                                                              | _(unchanged)_               | Schema validation at route level and inside service layer                       |
| Row-Level Security | **None — app-layer only**                                                        | Postgres RLS                | See consequence 3 below                                                         |
| Error Tracking     | Sentry                                                                           | _(unchanged)_               | Full context on errors — userId, orderId, route                                 |
| CI/CD              | GitHub Actions (`.github/workflows/ci.yml`)                                      | _(unchanged)_               | Lint → type-check → test → build                                                |
...
VendorType: STUDENT_COOK | CAMPUS_STALL | RESTAURANT | BAKERY
...
| role                  | UserRole        | BUYER / VENDOR / SUPER_ADMIN          |
| firstName             | String          |                                       |
| lastName              | String          |                                       |
...
| locationType           | LocationType       | ON_CAMPUS or OFF_CAMPUS                                                         |
| schoolName             | String?            | ON_CAMPUS only                                                                  |
| hostelOrStallName      | String?            | ON_CAMPUS only                                                                  |
| state                  | String?            | OFF_CAMPUS only                                                                 |
| areaOrAddress          | String?            | OFF_CAMPUS only                                                                 |
...
> The original text read: _"Vendor cannot appear in the marketplace until `profileCompleteness = 100`.
> It reaches 100 automatically when all checklist items are completed. No manual approval needed."_
...
`INCOMPLETE` → _(vendor submits)_ → `PENDING_REVIEW` → admin **approve** → `ACTIVE`
→ admin **reject(reason)** → `CHANGES_REQUESTED` → _(vendor resubmits)_ → `PENDING_REVIEW`
...
| `location`     | `locationType` set (OFF_CAMPUS also needs state \+ area \+ ≥1 campus) |
...
| vendorAmountKobo / vendorSettlementKobo | Int                | foodSubtotalKobo − prechopCommissionKobo (\+ delivery). **CORRECTED:** _not_ `amountKobo − platformFeeKobo` — `amountKobo` also contains the buyer's fee, which was never the vendor's money                                                                                                                                   |
...
| title     | String      |                                                     |
| body      | String      |                                                     |
| type      | String      | ORDER_PLACED / ORDER_READY / ORDER_CANCELLED / etc. |
...
> _"No UPDATE or DELETE permissions granted to the application database user — enforced at the
> Supabase RLS level."_ **Postgres and RLS are gone** (see §5); the store is MongoDB. Audit-log
...
| action        | String      | ORDER_STATUS_CHANGED / PAYMENT_VERIFIED / VENDOR_SUSPENDED / etc. |
| resourceType  | String      | buyer_order / vendor_profile / payment / etc.                     |
| resourceId    | String      |                                                                   |
...
> **⚠️ CORRECTED (2026-07-15).** The original text read: _"No approval gate. Vendor goes live on the
> marketplace automatically when `profileCompleteness` reaches 100%."_ **Reversed — there is a manual
...
> **⚠️ CORRECTED (2026-07-15).** The original text added: _"Vendor cannot appear in the marketplace
> until `profileCompleteness = 100`."_ **The percentages in the table below are a progress indicator,
...
> "timetable" below are _not_ on the submission checklist and cannot be completed before approval:
...
> balance _of_ — the number would always be fiction. PreChop also does not integrate Paystack's
...
- Future days show items as a preview labelled: _"Opens at \[vendor's usual cutoff time\]"_
...
_"Use your Thursday timetable? — Semo \+ Egusi, Pounded Yam \+ Oha, Zobo"_
...
- Title: auto-suggested from timetable (editable), e.g. _"Thursday Special — Semo & Egusi"_
...
- Live cutoff countdown timer (e.g. _"Closes in 1h 45m"_)
...
> **⚠️ CORRECTED (2026-07-15) — this row said "fewer than 5 completed _orders_", contradicting
...
> exists to stop, and orders are not a measure of how much _rating_ evidence exists. The two sections
...
> number in the response body). Ungated vendors also sort _below_ every rated vendor, never above.
...
- Server creates `BuyerOrder` (status: PENDING_PAYMENT), initialises Paystack transaction
...
- Vendor marks order READY → buyer notified via SMS: _"Your order is ready — come collect\!"_
...
> product.** The original text read: _"Buyer pays ₦50 … Vendor pays ₦100 … PreChop collects ₦150
> total per order."_ **The percentage model below is the real one** and is what the code charges.
...
- PreChop's take per order = buyer service fee \+ vendor commission (**≈11% of the food subtotal**,
  with the buyer half capped at ₦200 — _not_ a flat ₦150).
...
because the failure here is not a crash — it is a _silent wrong charge_. An **absent** field falls
...
→ Creates BuyerOrder { status: PENDING_PAYMENT }
...
| :--------------------- | :------------- | :----------------------- |
| PENDING_PAYMENT        | N/A            | Never paid               |
...
refund, forever**. The listing-closing sweep only closes the _listing_; the paid buyer orders
...
24 hours after `BuyerOrder.status` reaches COMPLETED, the buyer receives an in-app notification: _"How was your order from \[Vendor\]? Leave a quick review."_ Review window: 48 hours from notification. After that, the option expires permanently.
...
> **⚠️ CORRECTED (2026-07-15).** The original text read: _"`campusId` is enforced on all queries via
> Prisma middleware — cross-campus data access is impossible at the ORM layer."_ **There is no Prisma
...
| 7   | Vendor approval gate           | 🔄 **REVERSED 2026-07-15** | ~~No manual approval — auto-ACTIVE when profileCompleteness \= 100%~~ → **Manual admin approval gate.** INCOMPLETE → (submit) → PENDING_REVIEW → admin approve (→ACTIVE) / reject(reason) (→CHANGES_REQUESTED) → resubmit. Completeness no longer auto-activates. Submission gates on the **onboarding checklist**, not on completeness ≥100 — that score requires menu items \+ timetable, which are themselves behind the active-vendor gate, so scoring on 100% **deadlocked every applicant at ~60%**. `profileCompletenessRequired` is informational only. See §6                                      |
| 8   | User roles                     | ✅ Decided                 | Three roles only: BUYER, VENDOR, SUPER_ADMIN                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 9   | Location model                 | ✅ Decided                 | Campus-scoped. ON_CAMPUS: school \+ hostel/stall. OFF_CAMPUS: state \+ area                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
_PreChop PRD v3.0 — Final, with as-built corrections applied 2026-07-15_  
_Auth: Phone \+ OTP · Platform fee: 3% buyer (cap ₦200) \+ 8% vendor · Vendor go-live: manual admin approval_  
_Stack (as built): Next.js 16 App Router · MongoDB \+ Mongoose · Redis · node-cron · Paystack · Sendchamp · Resend · AWS S3_
...
> reversal is traceable — a silent edit loses the _why_ and invites someone to reverse it back. Where
