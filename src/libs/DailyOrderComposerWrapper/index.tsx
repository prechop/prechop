  ...
  async function seedFromTemplate() {
    try {
      const entries = await apiData<TemplateEntry[]>(
        api.get(`/timetable/template?dayOfWeek=${weekdayOf(scheduledDate)}`),
      );
      const ids = entries
        .map((e) => e.menuItem?.id ?? e.menuItem?._id)
        .filter((x): x is string => !!x);
      ...
      toast(`Added ${ids.length} item(s) from today's timetable`, "success");
    } catch (e) {
    ...
    return (
      <FadeIn>
        <Stack $gap={20}>
          <SuccessHero>
            ...
            <Stack $gap={6}>
              ...
              <Text $size={14} style={{ color: "rgba(255,255,255,0.9)" }}>
                “{published.title}” is open for orders. Share the link so buyers
                can order.
              </Text>
            </Stack>
          </SuccessHero>
          ...
          <Card>
            <Stack $gap={14}>
              <SectionHeader title="Share your order link" icon="🔗" />
              <LinkBox>
      ...
      <Stack $gap={20}>
        ...
        <Card>
          <Stack $gap={16}>
            ...
            <Text $muted $size={12}>
              Before it opens, buyers see this listing as “coming soon”. After
              it closes it’s pulled from the main page.
            </Text>
          ...
          <Stack $gap={14}>
            <ItemsHeader>
              <ItemsTitle>
                ...
                <span>Items</span>
                {selectedCount > 0 && (
                  <Badge $tone="primary">{selectedCount} selected</Badge>
                )}
              </ItemsTitle>
              <ItemsAction>
                <Button
            ...
            <Stack $gap={8}>
              {menuItems.map((m) => {
                ...
                return (
                  <ItemRow key={m.id} $on={on} onClick={() => toggle(m.id)}>
                    <Stack $gap={on ? 12 : 0}>
                      ...
                      {on && (
                        <QtyWrap onClick={(e) => e.stopPropagation()}>
                          <Input
                      ...
                      {on && attachedGroups(m).length > 0 && (
                        <ItemGroupsEditor
                          groups={attachedGroups(m)}
                          excluded={excludedGroups[m.id] ?? EMPTY_SET}
                          edits={optionEdits[m.id] ?? EMPTY_EDITS}
                          onToggle={(gid) => toggleGroupForItem(m.id, gid)}
                          onChangeOptions={(gid, options) =>
                            setGroupOptions(m.id, gid, options)
                          }
                        />
                      )}
                    </Stack>
