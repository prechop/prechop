...
export default function ForgotPinFlow({
	...
	async function requestOtp() {
		...
		try {
			const res = await api.post(
				"/vendors/me/security/forgot-pin/request",
				{ email },
			);
			setDevOtp((res.data as { data: { devOtp?: string } })?.data?.devOtp ?? null);
			setStep("verify");
	...
	async function verifyOtp() {
		...
		try {
			const res = await api.post(
				"/vendors/me/security/forgot-pin/verify",
				{ email, otp: otp.trim() },
			);
	...
	async function requestSupport() {
		if (!supportReason.trim() || supportReason.trim().length < 10) {
			toast("Please provide at least 10 characters explaining your issue.", "error");
			return;
		}
		setLoading(true);
		try {
			...
			setStep("done");
			toast("Support request submitted. An admin will review your identity.", "success");
			onDone?.();
		} catch (e) {
			toast(errMsg(e), "error");
		} finally {
		...
		<Stack $gap={12}>
			...
			{step === "done" && (
				<Stack $gap={10}>
					...
					<Text $muted $size={13}>
						Your request has been processed. Check your email or support
						updates for next steps.
					</Text>
					{onCancel && (
						<Button $variant="ghost" onClick={onCancel}>
							Close
