...
function orderBelongsInColumn(order: PipelineOrder, column: BoardColumn) {
  if (
    canonicalOrderStatus(order.status, order.fulfillmentType) !== column.status
  )
    return false;
  if (
...
export default function PipelineWrapper() {
  ...
  async function advance(order: PipelineOrder) {
    ...
    try {
      ...
      toast(`Order ${order.orderNumber} -> ${statusLabel(next.to)}`, "success");
      await Promise.all([
        mutate(),
  ...
  const filteredCompletedOrders = completedOrders
    .filter((order) => completedOrderMatchesFilter(order, completedFilter, now))
        ...
        <Stack $gap={20}>
          <PageHeader
            ...
            actions={
              liveCount > 0 ? (
                <Badge $tone="primary">
                  <FiClock size={14} aria-hidden /> {liveCount} live
                </Badge>
              ) : undefined
            }
          />
          ...
          <Select
            ...
            {active.map((dailyOrder) => (
              <option key={dailyOrder.id} value={dailyOrder.id}>
                {dailyOrder.title} - {dailyOrder.totalOrdersCount} order
          ...
          {ordersLoading ? (
            <Board>
              {COLUMNS.map((column) => (
                <Lane key={column.key} $accent={LANE_ACCENT[column.status]}>
                  <LaneHead>
                    <LaneTitle>
                      <LaneIcon>
                        <column.icon size={16} aria-hidden />
                      </LaneIcon>
                      {column.label}
                    </LaneTitle>
                  </LaneHead>
                ...
                return (
                  <Lane key={column.key} $accent={LANE_ACCENT[column.status]}>
                    <LaneHead>
                      ...
                      <Badge $tone={statusTone(column.status)}>
                        {columnOrders.length}
                      </Badge>
                    </LaneHead>

                    {columnOrders.length === 0 ? (
                      <LaneEmpty>{column.empty}</LaneEmpty>
                    ) : (
                      columnOrders.map((order) => {
                        const next = actionForOrder(order);
                        const NextIcon = next ? actionIcon(next.to) : null;
                        const handoverEligible = isBuyerHandoverEligible(
                          order.status,
                          order.fulfillmentType,
                          order.handoverCredentialUsedAt,
                        );
                        const countdown = acceptanceCountdown(
                            ...
                            <Stack $gap={10}>
                              <Row
                                ...
                                <Stack $gap={4}>
                                  ...
                                  <Badge
                                    ...
                                    {order.fulfillmentType === "DELIVERY" ? (
                                      <>
                                        <FiTruck size={14} aria-hidden />{" "}
                                        Delivery
                                      </>
                                    ) : (
                                      <>
                                        <FiPackage size={14} aria-hidden />{" "}
                                        Pickup
                                      </>
                                    )}
                                  </Badge>
                              ...
                              <Stack $gap={3}>
                                {order.items.map((item, index) => (
                                  <Row key={index} $gap={8} $align="baseline">
                                    <Text $size={14} $weight={700}>
                                      {item.quantity}x
                                    </Text>
                                    <Text $size={14}>{item.snapshotName}</Text>
                                  </Row>
                                ))}
                              </Stack>
                              ...
                              {order.fulfillmentType === "DELIVERY" && (
                                <AddrLine>
                                  ...
                                  <span>{deliveryAddress(order)}</span>
                                </AddrLine>
                              )}
                                ...
                                "AWAITING_VENDOR_ACCEPTANCE" ? (
                                  <>
                                    ...
                                    <Button
                                      ...
                                      <FiXCircle size={14} aria-hidden /> Reject
                                      order
                                    </Button>
                                  </>
                                ) : handoverEligible ? (
            ...
            <HistoryPanel>
              <Stack $gap={14}>
                <Row $justify="space-between" $align="center" $gap={12}>
                  <Stack $gap={2}>
                    <Text $weight={800}>Completed orders</Text>
                    <Text $muted $size={13}>
                      Recent fulfilled orders are kept out of the live board.
                    </Text>
                  </Stack>
                  <FilterRow>
                    <FilterButton
                      type="button"
                      $active={completedFilter === "today"}
                ...
                {filteredCompletedOrders.length === 0 ? (
                  <LaneEmpty>No completed orders in this filter</LaneEmpty>
                ) : (
                  <HistoryList>
                    {filteredCompletedOrders.map((order) => (
                      <OrderCard key={order.id}>
                        <Stack $gap={8}>
                          ...
                          <Badge $tone="success">
                            <FiCheckCircle size={14} aria-hidden /> Completed
                          </Badge>
                          <Text $muted $size={13}>
                            {order.items
                              .map(
                ...
                {completedFilter !== "all" && completedOrders.length > 6 && (
