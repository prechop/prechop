        ...
        <Card style={{ flex: "1 1 320px" }}>
          <Stack $gap={12}>
            ...
            {requests.map((request) => (
              <Button
                key={request.id}
                $variant={selected?.id === request.id ? "secondary" : "ghost"}
                ...
                <Stack $gap={2}>
                  <Row $gap={8} $align="center" $wrap>
                    <Text $weight={800}>{request.subject}</Text>
                    <Badge $tone={tone(request.status)}>{request.status}</Badge>
                  </Row>
                  <Text $muted $size={12}>
                    {request.senderRole} · {request.category} ·{" "}
            ...
            <Stack $gap={14}>
              <Row $justify="space-between" $align="center" $wrap>
                <Stack $gap={3}>
                  ...
                  <Text $muted $size={13}>
                    {selected.senderRole} · {selected.category} · account{" "}
                    {selected.userId}
                  </Text>
                </Stack>
                <Badge $tone={tone(selected.status)}>{selected.status}</Badge>
              </Row>
              {(selected.relatedOrderRef || selected.relatedPaymentRef) && (
                <Text $muted $size={13}>
              ...
              <Row $gap={10} $wrap>
                ...
                <Button $variant="ghost" onClick={assignToMe} disabled={busy}>
                  Assign to me
                </Button>
              </Row>
            </Stack>
