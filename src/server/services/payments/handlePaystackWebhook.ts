...
export async function handlePaystackWebhook({
  ...
  const { reference, amount, requested_amount, channel, status } = event.data;
  ...
  const paidAmount = requested_amount ?? amount;
  if (paidAmount !== payment.amountKobo) throw ErrPaymentAmountMismatch;
