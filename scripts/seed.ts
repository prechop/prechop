...
const NORMALIZED_ADMIN_EMAIL = SEED_ADMIN_EMAIL?.trim().toLowerCase() ?? "";
...
async function ensureCampus(input: {
	...
	if (existing) {
		if (!existing.isPublic) {
			await updateCampusDB({
				id: existing._id.toString(),
				payload: { isPublic: input.isPublic ?? true },
			});
			log(`campus ${input.shortCode} marked public`);
		}
	...
	const created = await createCampusDB({
		payload: { ...input, isPublic: input.isPublic ?? true },
	});
...
async function ensureAdministrator(input: {
	...
	const administratorsGroupId = await getBuiltInGroupId(ADMINISTRATORS_GROUP);

	if (!administratorsGroupId) {
		throw new Error(`Built-in group not found: ${ADMINISTRATORS_GROUP}`);
	}

	log(`Administrators group resolved: ${administratorsGroupId}`);
	...
	if (!created) {
		throw new Error(`Failed to create administrator: ${input.email}`);
	}
...
/**
 * Backfill permanent short vendor IDs and Brand Kit payment status for
 * existing ACTIVE vendors so the new feature gate does not unexpectedly
 * block kitchens that were already operating.
 */
async function backfillVendorShortIdsAndBrandKit(): Promise<void> {
	const vendors = await listVendorsDB({
		status: VendorStatus.ACTIVE,
		limit: 500,
	});
	if (vendors.length === 0) {
		log("no active vendors to backfill");
		return;
	}
...
function generateUniqueShortId(
	...
	for (let i = 0; i < 999; i += 1) {
		const candidate = `${prefix.slice(0, 2)}${String.fromCharCode(65 + (i % 26))}`;
...
async function main(): Promise<void> {
	...
	await backfillVendorShortIdsAndBrandKit();
...
// //         status: VendorStatus.ACTIVE,
// //         locationType: LocationType.ON_CAMPUS,
// //         description: "Home-style Nigerian meals, cooked fresh to order.",
// //         categories: [MenuCategory.MEALS, MenuCategory.DRINKS],
// //         isOpenForOrders: true,
...
// //           optionGroupIds:
// //             menuSpecs[i].category === MenuCategory.MEALS ? mealGroupIds : [],
// //           ...menuSpecs[i],
// //         },
// //       });
...
// import { paystackProvider } from "../src/server/providers";
// import { getBuiltInGroupId, seedBuiltInIam } from "../src/server/services/iam";
// export const SEED_ADMIN_PHONE = process.env.SEED_ADMIN_PHONE ?? "08130135756";
...
// }): Promise<{ _id: string; created: boolean }> {
// 	const groupId = await getBuiltInGroupId(input.groupName);
// 	const email = (input.email ?? seedEmail(input)).trim().toLowerCase();
// 	const existing = await getUserByEmailDB({ email });
