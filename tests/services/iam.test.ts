...
import { resolveUserGroupNames } from "@/server/lib/auth";
import {
  clearCollections,
  connectTestDB,
  dropAndDisconnect,
...
describe("seedBuiltInIam + resolvePermissions", () => {
  ...
  it("resolves current database group membership for auth checks", async () => {
    const user = await makeUser();
    const id = user!._id.toString();
    const adminGroup = await getBuiltInGroupId(ADMINISTRATORS_GROUP);
    await setUserGroups({ targetId: id, groupIds: [adminGroup!], actor });

    const groups = await resolveUserGroupNames(id);
    expect(groups).toContain(ADMINISTRATORS_GROUP);
  });
});
...
describe("policy service", () => {
  ...
  it("rejects unknown actions", async () => {
    await expect(
      createPolicy({
        name: "Bad",
        statements: [{ effect: "Allow", actions: ["not:a:real:action"] }],
        actor,
      }),
    ).rejects.toThrow();
  });
