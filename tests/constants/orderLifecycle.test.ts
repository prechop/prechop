...
describe("order lifecycle mapping", () => {
	...
	it("exposes handover only at pickup ready or delivery in transit", () => {
		...
		expect(
			isBuyerHandoverEligible("READY_FOR_PICKUP", "PICKUP", null),
		).toBe(true);
		expect(
			isBuyerHandoverEligible("READY_FOR_DELIVERY", "DELIVERY", null),
		).toBe(false);
		expect(isBuyerHandoverEligible("IN_TRANSIT", "DELIVERY", null)).toBe(
			true,
		);
		expect(
			isBuyerHandoverEligible("IN_TRANSIT", "DELIVERY", new Date()),
		).toBe(false);
	});
	...
	it("returns clear handover unavailable messages", () => {
		...
		expect(handoverUnavailableMessage("DELIVERY")).toBe(
			"Handover code becomes available when the order is on the way.",
		);
	});
});
